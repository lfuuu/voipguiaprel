...
var PricelistViewCtrl = function ($scope, Redirect, List, Pricelist, PricelistLocation, PricelistFilterA, PricelistFilterB, PricelistPrefixPrice, params, $modalInstance, $window) {
  ...
  $scope.drawTable = function (data) {
    ...
    if (pl) {
      ...
      $scope.pricelistDefaultTarificationFreeSeconds     = pl.default_tarification_free_seconds;
      $scope.pricelistDefaultTarificationIntervalSeconds = pl.default_tarification_interval_seconds;
      $scope.pricelistDefaultTarificationMinPaidSeconds  = pl.default_tarification_min_paid_seconds;
      $scope.pricelistDefaultTarificationType            = pl.default_tarification_type;

      // Критично: привести к числу
  ...
  $scope.initData = function (id) {
    Pricelist.getWithDependentsNew({id: id}).then(function (data) {
      // API возвращает массив-«плоскую таблицу»
  ...
  $scope.getPrefixPriceList = function (filter_b_id, page) {
    PricelistPrefixPrice.read({pricelist_filter_b_id: filter_b_id, page_number: page}).then(function (data) {
      ...
      for (var j = $scope.limit - 1; j >= 0; j--) {
        var item = $scope.prefixes[filter_b_id][j];
  ...
  $scope.addLocation = function (pricelistId) {
    // передаём и тип услуги, и активность
    Redirect.pricelistLocationCreate(pricelistId, $scope.pricelistIsActive, $scope.pricelistServiceTypeId).then(function () {
  ...
  $scope.addFilterB = function (filterAId) {
    Redirect.pricelistFilterBCreate(
      ...
      $scope.pricelistDateStart // было $scope.item[2].date_start
    ).then(function () {
      $scope.initData($scope.item.id);
    }, function () {
      $scope.initData($scope.item.id);
  ...
  $scope.editFilterB = function (id) {
    Redirect.pricelistFilterBEdit(id, $scope.pricelistIsActive, $scope.pricelistDateStart).then(function () {
      $scope.initData($scope.item.id);
    }, function () {
      $scope.initData($scope.item.id);
    });
  ...
  $scope.deleteLocation = function (id) {
    ...
    PricelistLocation.delete(id).then(function() {
      $scope.initData($scope.item.id);
    });
  };
  ...
  $scope.deleteFilterA = function (id) {
    ...
    PricelistFilterA.delete(id).then(function() {
      $scope.initData($scope.item.id);
    });
  };

  $scope.deleteFilterB = function (item) {
    var message = item.has_children
      ? 'Вы уверены, что хотите удалить фильтр B и ВСЕ созданные в нем прайсы префиксов?'
      : 'Удалить фильтр B?';

    if (!$window.confirm(message)) return;

    PricelistFilterB.delete(item.id).then(function() {
      $scope.initData($scope.item.id);
    });
  };
  ...
  $scope.searchPrefixById = function(searchValue, filterBId) {
    ...
    PricelistPrefixPrice.readAll(requestData).then(function(data) {
      var index = data.findIndex(function(it) {
        return it.prefix_b && it.prefix_b.trim() === searchValue.trim();
      });
      if (index !== -1) {
        var pageNumber = Math.floor(index / $scope.limit) + 1;
        $scope.list.forEach(function(row) {
          if (row.filter_b_id === filterBId) {
  ...
  $scope.deletePrefixPrice = function (id) {
    ...
    PricelistPrefixPrice.delete(id).then(function() {
      $scope.initData($scope.item.id);
    });
  };
