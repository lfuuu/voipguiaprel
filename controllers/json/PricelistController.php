...
class PricelistController extends JsonController
{
    ...
    public function actionRead()
    {
        ...
        $count = $query->count(); // групповой count по текущему запросу
    ...
    public function actionGetWithDependentsNew()
    {
        ...
        // Список полей как было (оставляем контракт ответа неизменным)
        $flatRules = [
        ...
        /**
         * ВАЖНО: оставляем полную выдачу по Location/A/B,
         * но резко уменьшаем «раздувание» по префиксам.
         *
         * Раньше: брались DISTINCT prefix_b (LIMIT :limit), затем JOIN всех строк по этим prefix_b,
         * что размножало строки по разным date_from и т.п.
         *
         * Теперь: на каждый prefix_b берётся РОВНО ОДНА — самая свежая — строка (date_from DESC),
         * и только до PAGE_LIMIT префиксов на каждый Filter B.
         */
        $prefixPriceSelect = <<<SQL
LATERAL (
    SELECT ppp.*
    FROM (
        SELECT DISTINCT ON (ppp2.prefix_b)
               ppp2.*
        FROM billing_uu.pricelist_prefix_price ppp2
        WHERE ppp2.pricelist_filter_b_id = pfb.id
          AND ppp2.date_to > now()
        ORDER BY ppp2.prefix_b, ppp2.date_from DESC, ppp2.id DESC
    ) AS ppp
    ORDER BY ppp.prefix_b
    LIMIT :limit
) 
SQL;
        ...
        $rows = Pricelist::find()
            ...
            // сохраняем LATERAL, но уже «ужатый» по количеству строк
        ...
        if ($debug) {
            ...
            $db   = \Yii::$app->db;
            $plCnt = (int)$db->createCommand(
                'SELECT COUNT(*) FROM billing_uu.pricelist_location WHERE pricelist_id = :id',
                [':id' => $id]
            )->queryScalar();

            $faCnt = (int)$db->createCommand(
                'SELECT COUNT(*) FROM billing_uu.pricelist_filter_a a 
                 JOIN billing_uu.pricelist_location l ON l.id = a.pricelist_location_id 
                 WHERE l.pricelist_id = :id',
                [':id' => $id]
            )->queryScalar();

            $fbCnt = (int)$db->createCommand(
                'SELECT COUNT(*) FROM billing_uu.pricelist_filter_b b 
                 JOIN billing_uu.pricelist_filter_a a ON a.id = b.pricelist_filter_a_id 
                 JOIN billing_uu.pricelist_location l ON l.id = a.pricelist_location_id 
                 WHERE l.pricelist_id = :id',
                [':id' => $id]
            )->queryScalar();
            ...
            $ppCnt = (int)$db->createCommand(
                'SELECT COUNT(*) 
                 ...
                 JOIN billing_uu.pricelist_filter_b b ON b.id = ppp.pricelist_filter_b_id 
                 JOIN billing_uu.pricelist_filter_a a ON a.id = b.pricelist_filter_a_id 
                 JOIN billing_uu.pricelist_location l ON l.id = a.pricelist_location_id 
                 WHERE l.pricelist_id = :id AND (ppp.date_to > now() OR ppp.prefix_b IS NULL)',
                [':id' => $id]
            )->queryScalar();
        ...
        // Контракт не меняем: используем тот же вью-строитель
        $result = $isShort
            ...
            foreach ($filterBArray as $filterB) {
                ...
                foreach ($filterB->prefixPriceBasic as $prefixPrice) {
                    if (!isset($prefixesToSave[$prefixPrice->prefix_b])) {
                        $prefixesToSave[$prefixPrice->prefix_b] = [
                            'date_from' => $prefixPrice->date_from,
                            'id' => $prefixPrice->id
                        ];
                    } else {
    ...
    public function actionCopy()
    {
        ...
        $result = (new Query())
            ->select(['id' => new Expression('billing_uu.clone_pricelist(:old_pricelist_id)')])
    ...
    public function actionUpdatePrefixPrices()
    {
        ...
        if (
            !preg_match('~^\d{4}-\d{2}-\d{2}$~', $this->request['dateFrom']) ||
            !preg_match('~^\d{4}-\d{2}-\d{2}$~', $this->request['dateTo'])
        ) {
    ...
    public function actionSearch()
    {
        ...
        $fields = [
            'a_country_id', 'b_country_id', 'a_region_id', 'b_region_id', 'a_city_id', 'b_city_id',
            'a_operator_id', 'b_operator_id', 'a_ndc_id', 'b_ndc_id', 'timestamp', 'number_a', 'number_b',
            'mcc', 'mnc', 'location_id', 'service_type_id'
        ];
    ...
    // ======== служебные приватные методы, если они у вас есть в реальном файле ========
