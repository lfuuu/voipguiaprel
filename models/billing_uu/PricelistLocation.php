...
class PricelistLocation extends \yii\db\ActiveRecord
{
    ...
    public function getFilterA()
    {
        return $this->hasMany(PricelistFilterA::className(), ['pricelist_location_id' => 'id'])
            ...
            ->leftJoin('(select b.id, string_agg(name_rus, \', \') as nnp_country_name, string_agg(name_eng, \', \') as nnp_country_name_eng from nnp.country c join billing_uu.pricelist_filter_a b on c.code = any(b.nnp_country) group by b.id) as c', 'c.id = billing_uu.pricelist_filter_a.id')
