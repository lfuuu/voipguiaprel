...
{
    // множества вместо in_array()
    $locationsProcessed = [];
    $filtersAProcessed  = [];
    $filtersBProcessed  = [];

    // массивы ID для последующего резолва имён
    $mccIdArray = [];
    ...
    // ВАЖНО: вход уже отсортирован SQL: pl.id, pfa.id, pfb.id, ppp.prefix_b, ppp.id
    // self::sortAlphabetically($queryResult, $idArrays); // — убрано
    ...
    // карта для быстрого поиска строки по prefix_b внутри текущего filterB-блока
    $rowIndexByPrefixB = [];
    $currentFilterBId = null;

    foreach ($queryResult as $queryItem) {
        // 1) Сбор ID для справочников (как было, но без in_array — используем множества выше)
        if (!empty($queryItem['pl__id']) && !isset($locationsProcessed[$queryItem['pl__id']])) {
            self::processQueryArray($mccIdArray,             $queryItem['pl__mcc']);
            ...
            $locationsProcessed[$queryItem['pl__id']] = true;
        }

        if (!empty($queryItem['pfa__id']) && !isset($filtersAProcessed[$queryItem['pfa__id']])) {
            self::processQueryArray($nnpCountryIdArray,      $queryItem['pfa__nnp_country']);
            self::processQueryArray($nnpDestinationIdArray,  $queryItem['pfa__nnp_destination']);
            self::processQueryArray($nnpOperatorIdArray,     $queryItem['pfa__nnp_operator']);
            self::processQueryArray($nnpRegionIdArray,       $queryItem['pfa__nnp_region']);
            self::processQueryArray($nnpCityIdArray,         $queryItem['pfa__nnp_city']);
            self::processQueryArray($nnpNdcTypeIdArray,      $queryItem['pfa__nnp_ndc_type']);
            $filtersAProcessed[$queryItem['pfa__id']] = true;
        }

        if (!empty($queryItem['pfb__id']) && !isset($filtersBProcessed[$queryItem['pfb__id']])) {
            self::processQueryArray($nnpCountryIdArray,      $queryItem['pfb__nnp_country']);
            self::processQueryArray($nnpDestinationIdArray,  $queryItem['pfb__nnp_destination']);
            self::processQueryArray($nnpOperatorIdArray,     $queryItem['pfb__nnp_operator']);
            self::processQueryArray($nnpRegionIdArray,       $queryItem['pfb__nnp_region']);
            self::processQueryArray($nnpCityIdArray,         $queryItem['pfb__nnp_city']);
            self::processQueryArray($nnpNdcTypeIdArray,      $queryItem['pfb__nnp_ndc_type']);
            $filtersBProcessed[$queryItem['pfb__id']] = true;
        }

        // 2) Инициализация корневой строки прайс-листа
        if (empty($result)) {
        ...
        // 4) Filter A + совместный заголовок с Filter B и префиксами
        if (empty($queryItem['pfa__id'])) {
            continue;
        }

        // Используем предвычисленный счётчик distinct(prefix_b) из основного запроса:
        $realCount = (int)($queryItem['pfb__total_prefix_count'] ?? 0);
        $pageLimit = (int) PricelistPrefixPrice::PAGE_LIMIT;
        $countForHeader = ($realCount >= $pageLimit) ? ($pageLimit + 1) : $realCount;
        ...
        if (!isset($result[$filterAKey]['is_filter_a_header']) ||
            ...
            if ($countForHeader > 0) {
                $result[$counter] = self::createFilterAFilterBPrefixRow($queryItem, $idArrays, $countForHeader, $realCount, $alphaNames);
                $filterAKey  = $counter;
                $filterBKey  = $counter;
                $counter++;

                // Смена блока filterB — обнуляем карту строк по префиксу
                $currentFilterBId = $queryItem['pfb__id'] ?? null;
                $rowIndexByPrefixB = [];
            }
        }
        ...
        // При смене pfb блока — тоже сбрасываем карту
        if ($currentFilterBId !== $queryItem['pfb__id']) {
            $currentFilterBId = $queryItem['pfb__id'];
            $rowIndexByPrefixB = [];
        }

        $countForBHeader = $countForHeader; // те же расчёты
        ...
        if (!isset($result[$filterBKey]['is_filter_b_header']) ||
            ...
            if ($countForBHeader > 0) {
                $result[$counter] = self::createFilterBPrefixRow($queryItem, $idArrays, $countForBHeader, $realCount);
                $filterBKey = $counter;
                $counter++;

                // аккумулируем в A-заголовок
                if (isset($result[$filterAKey]['total_prefix_count'])) {
                    $result[$filterAKey]['total_prefix_count'] += $countForBHeader;
                }

                // новый блок префиксов — заново карта
                $rowIndexByPrefixB = [];
            }
        }
        ...
        // если последний добавленный элемент — это не тот же prefix, ищем/добавляем через карту
        $currentPrefixKey = $queryItem['ppp__prefix_b'] . ' ';

        // если предыдущая строка — не этот же prefix_id, используем карту
        if (!isset($result[$counter - 1]['prefixes'][0]['prefix_price_id']) ||
            $result[$counter - 1]['prefixes'][0]['prefix_price_id'] != $queryItem['ppp__id']) {

            $isPrefixSet = false;

            if (isset($rowIndexByPrefixB[$currentPrefixKey])) {
                // строка для этого префикса уже есть — просто доклеиваем
                $idx = $rowIndexByPrefixB[$currentPrefixKey];
                $result[$idx]['prefixes'][] = self::createPrefixItem($queryItem, $result[$idx]['prefixes']);
                // порядок уже корректный (ORDER BY ppp.id), глобальной сортировки не делаем
                self::recalcPrefixesDynamics($result[$idx]['prefixes']);
                $isPrefixSet = true;
            }

            if (!$isPrefixSet && ($counter - $filterBKey) < $pageLimit) {
                // создаём новую строку префикса
                $result[$counter] = self::createPrefixRow($queryItem);
                $rowIndexByPrefixB[$currentPrefixKey] = $counter; // запоминаем индекс строки для быстрого доступа
                $counter++;

                // если достигли лимит — вставим футер с общим realCount без лишнего запроса
                if (($counter - $filterBKey) == $pageLimit) {
    ...
    // Разрешение имён для собранных IDs (как было)
    foreach ($idArrays as $key => &$item) {
        $item['ids'] = array_unique($item['ids']);
        if (count($item['ids'])) {
            $tempIds = (new Query())
